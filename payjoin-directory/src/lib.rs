...
impl<D: Db> hyper::service::Service<Request<Incoming>> for Service<D> {
    ...
    fn call(&self, req: Request<Incoming>) -> Self::Future {
        ...
        let this = Service::new(db, ohttp);
        Box::pin(async move { this.serve(req).await })
    }
}

impl<D: Db> Service<D> {
    pub fn new(db: D, ohttp: ohttp::Server) -> Self { Self { db, ohttp } }
...
fn handle_peek<Error: db::SendableError>(
    result: Result<Vec<u8>, db::Error<Error>>,
    timeout_response: Response<BoxBody<Bytes, hyper::Error>>,
) -> Result<Response<BoxBody<Bytes, hyper::Error>>, HandlerError> {
    match result {
        Ok(payload) => Ok(Response::new(full(payload))),
        Err(e) => match e {
