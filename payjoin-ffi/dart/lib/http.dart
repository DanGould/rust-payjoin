...
Future<OhttpKeys> fetchOhttpKeys({
  ...
  RawSocket socket = await (relayIsHttps
      ...
      .timeout(timeout, onTimeout: () => throw HttpException('relay connect timeout'));
  ...
  try {
    ...
    if (destIsHttps) {
      ...
      ).timeout(timeout, onTimeout: () => throw HttpException('directory TLS handshake timeout'));
    }
...
Future<StreamSubscription<RawSocketEvent>> _openConnectTunnel(
  ...
  final headers = await done.future.timeout(timeout,
...
Future<_Response> _sendGet(
  ...
  final responseBytes = await done.future.timeout(timeout,
...
_Response _parseResponse(Uint8List responseBytes) {
  ...
  if (hasCL) {
    ...
    throw HttpException('No framing header (Content-Length or Transfer-Encoding required)');
  }
}
