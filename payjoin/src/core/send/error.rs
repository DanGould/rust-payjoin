...
pub(crate) enum InternalProposalError {
    ...
    SenderTxinCisaFieldsChanged,
    SenderTxinNonAllSighashType,
...
impl fmt::Display for InternalProposalError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self {
            ...
            SenderTxinCisaFieldsChanged => write!(f, "an input in proposed transaction belonging to the sender has different signature aggregation fields than the original"),
            SenderTxinNonAllSighashType => write!(f, "an input in proposed transaction belonging to the sender requests a sighash type other than SIGHASH_ALL"),
...
impl std::error::Error for InternalProposalError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        ...
        match self {
            ...
            SenderTxinCisaFieldsChanged => None,
            SenderTxinNonAllSighashType => None,
